...
function getCachePaths(sourceFilename, cacheDir, packagesDir) {
    ...
    const localPackagePath = getLocalPackageFilePath(sourceFilename, cacheDir, packagesDir, this.context)
...
function writeCache(key, data, callback) {
    ...
    if (cacheDir) {
        ...
        content = content.replace(/require\("(.+)stylePostLoader\.js(.+)&lang=css&"\)/g, `require("$1stylePostLoader.js$2&lang=css&css")`)
